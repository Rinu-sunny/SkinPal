  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      try {
        ...
        if (!session?.user_id) {
          console.error('❌ No session or user_id - User must log in!')
          console.log('📍 Full localStorage:', {
            keys: Object.keys(localStorage),
            skinpalSession: localStorage.getItem('skinpal_session')
          })
          setHistory([])
          setIsLoading(false)
          return
        ...
        console.log(`📋 Loading history for user ${session.user_id}...`)
        ...
        console.log('📥 Response structure:', {
          ...
          historyType: typeof profile?.history,
          historyLength: profile?.history?.length || 'N/A'
        })
        
        if (!mounted) return
        
        const historyData = profile?.history || []
        console.log(`✅ Loaded ${historyData.length} analyses`)
        
        if (historyData.length === 0) {
          console.warn('⚠️  No history data returned from API')
        }
