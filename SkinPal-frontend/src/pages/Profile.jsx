...
const CalendarDay = memo(({ day, hasAnalysis, isToday }) => {
  if (!day) {
    return (
      <div
        style={{
...
const CalendarGrid = memo(({ getMonthCalendar }) => {
  return (
    <div style={{
      display: 'grid',
...
export default function Profile(){
  ...
  const user = useMemo(() => {
    ...
    const displayEmail = (profile?.email && profile.email.trim()) 
      ? profile.email 
      : (session?.email && session.email.trim()) 
        ? session.email 
        : 'Email not set'
    return {
      name: displayName,
    ...
    <div className='container profile-page'>
      ...
      {hasLoaded && (
        <div style={{
          ...
          minHeight: history.length === 0 ? '200px' : 'auto',
          display: 'flex',
          ...
          {history.length === 0 ? (
            <div style={{
              ...
              <p style={{ margin: '0 0 0.5rem 0' }}>📊 No skin analysis yet</p>
              <p style={{ margin: 0, fontSize: '0.8rem' }}>Take your first skin analysis to see it here!</p>
            </div>
          ) : (
            <div style={{ width: '100%' }}>
          {/* Calendar Header */}
