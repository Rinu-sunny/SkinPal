...
def analyze():
    ...
    try:
        ...
        print(f"📝 image length: {len(image_base64) if image_base64 else 0}")

        if not user_id or not image_base64:
            print(f"❌ Missing: user_id={bool(user_id)}, image_base64={bool(image_base64)}")
        ...
        image_id = None
        try:
            # First, verify user record exists (don't auto-create to avoid conflicts)
            print(f"🔍 Verifying user {user_id} exists...")
            user_check = supabase_service.table("users").select("user_id").eq("user_id", user_id).limit(1).execute()
            
            if not user_check.data:
                print(f"❌ User {user_id} not found in users table - user must sign up first")
